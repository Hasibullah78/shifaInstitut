                ...
                <div class="row">
                    ...
                    <div class="col-md-4">
                        <div class="dropdown mb-3 notprint" id="list">
                            ...
                            <a href="{{ url('student/mideterm_exam') }}">منځنی ازموینه</a>
                            <a href="{{ url('student/final_exam') }}">  وروستی ازموینه </a>
                            </div>
                            ...
                            {{-- <div class="dropdown" id="list">
                                ...
                                <a href="{{ url('select/subject'.$subject->id) }}">{{ $subject->name }}</a>
                ...
                <thead>
                    ...
                    <tr>
                        <div class="row">
                            ...
                            @endif
                            <a href="{{ url('student/mideterm_exam') }}" class="btn btn-primary">Back to Subjects</a>
                        </div>
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td style="text-align: center" >
                            {{ $subject->name }}
                            {{-- @php
                                $markss=ExamMark::all()->where('student_id','==',$student->id)->first();
                            @endphp

                            @if($markss)
                            <span class="">
                                @php
                                    $subjects=Subject::all()->where('id','==',$markss->subject_id);
                                @endphp
                                @foreach ($subjects as $subject)
                                {{ $subject->name }}
                                @endforeach --}}
                            ...
                            @endif
                            @if(!$markss)
                                <span class=" text-danger">مضمون نده انتخاب شوی</span>
                            @endif --}}
                      </td>
                      ...
                      $marks_s=ExamMark::all()->where('student_id','==',$student->id)->where('subject_id','==',$subject->id)->first();
                        ...
                        <td style="text-align: center">
                           ...
                           @if(!$marks_s)
                                ...
                                <a href="{{ route('AddMideterm.First',['student_id'=>$student->id,'subject_id'=>$subject->id]) }}" class="btn btn-primary">نمری اضافه کړی</a>
